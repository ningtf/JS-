...
function am1(t, i, r, o, n, e) {
    for (; --e >= 0;) {
        ...
        n = Math.floor(s / 67108864), r[o++] = 67108863 & s
    }
    return n
}
...
function am2(t, i, r, o, n, e) {
    for (var s = 32767 & i, h = i >> 15; --e >= 0;) {
        var p = 32767 & this[t],
            a = this[t++] >> 15,
            f = h * p + a * s;
        p = s * p + ((32767 & f) << 15) + r[o] + (1073741823 & n), n = (p >>> 30) + (f >>> 15) + h * a + (n >>> 30), r[o++] = 1073741823 & p
    }
    return n
}
...
function am3(t, i, r, o, n, e) {
    for (var s = 16383 & i, h = i >> 14; --e >= 0;) {
        var p = 16383 & this[t],
            a = this[t++] >> 14,
            f = h * p + a * s;
        p = s * p + ((16383 & f) << 14) + r[o] + n, n = (p >> 28) + (f >> 14) + h * a, r[o++] = 268435455 & p
    }
    return n
}
...
function bnpFromString(t, i) {
    ...
    for (var o = t.length, n = !1, e = 0; --o >= 0;) {
        ...
        0 > s ? "-" == t.charAt(o) && (n = !0) : (n = !1, 0 == e ? this[this.t++] = s : e + r > this.DB ? (this[this.t - 1] |= (s & (1 << this.DB - e) - 1) << e, this[this.t++] = s >> this.DB - e) : this[this.t - 1] |= s << e, e += r, e >= this.DB && (e -= this.DB))
    }
    8 == r && 0 != (128 & t[0]) && (this.s = -1, e > 0 && (this[this.t - 1] |= (1 << this.DB - e) - 1 << e)), this.clamp(), n && BigInteger.ZERO.subTo(this, this)
}
...
function bnToString(t) {
    ...
    if (s-- > 0) for (h < this.DB && (r = this[s] >> h) > 0 && (n = !0, e = int2char(r)); s >= 0;) i > h ? (r = (this[s] & (1 << h) - 1) << i - h, r |= this[--s] >> (h += this.DB - i)) : (r = this[s] >> (h -= i) & o, 0 >= h && (h += this.DB, --s)), r > 0 && (n = !0), n && (e += int2char(r));
    return n ? e : "0"
}
...
function bnpLShiftTo(t, i) {
    var r, o = t % this.DB,
        ...
        h = this.s << o & this.DM;
    for (r = this.t - 1; r >= 0; --r) i[r + s + 1] = this[r] >> n | h, h = (this[r] & e) << o;
...
function bnpRShiftTo(t, i) {
    ...
    var o = t % this.DB,
        ...
        e = (1 << o) - 1;
    i[0] = this[r] >> o;
    for (var s = r + 1; s < this.t; ++s) i[s - r - 1] |= (this[s] & e) << n, i[s - r] = this[s] >> o;
    o > 0 && (i[this.t - r - 1] |= (this.s & e) << n), i.t = this.t - r, i.clamp()
}

function bnpSubTo(t, i) {
    for (var r = 0, o = 0, n = Math.min(t.t, this.t); n > r;) o += this[r] - t[r], i[r++] = o & this.DM, o >>= this.DB;
    if (t.t < this.t) {
...
function bnpMultiplyTo(t, i) {
    var r = this.abs(),
        o = t.abs(),
        n = r.t;
    for (i.t = n + o.t; --n >= 0;) i[n] = 0;
    for (n = 0; n < o.t; ++n) i[n + r.t] = r.am(0, o[n], i, n, 0, r.t);
...
function bnpDivRemTo(t, i, r) {
    ...
    if (!(o.t <= 0)) {
        var n = this.abs();
        if (n.t < o.t) return null != i && i.fromInt(0), void(null != r && this.copyTo(r));
        null == r && (r = nbi());
        var e = nbi(),
            s = this.s,
            h = t.s,
            p = this.DB - nbits(o[o.t - 1]);
        p > 0 ? (o.lShiftTo(p, e), n.lShiftTo(p, r)) : (o.copyTo(e), n.copyTo(r));
        var a = e.t,
            f = e[a - 1];
        if (0 != f) {
            var u = f * (1 << this.F1) + (a > 1 ? e[a - 2] >> this.F2 : 0),
                ...
                b = v - a,
                l = null == i ? nbi() : i;
            for (e.dlShiftTo(b, l), r.compareTo(l) >= 0 && (r[r.t++] = 1, r.subTo(l, r)), BigInteger.ONE.dlShiftTo(a, l), l.subTo(e, e); e.t < a;) e[e.t++] = 0;
            for (; --b >= 0;) {
                var T = r[--v] == f ? this.DM : Math.floor(r[v] * g + (r[v - 1] + c) * m);
                if ((r[v] += e.am(0, T, r, b, 0, a)) < T) for (e.dlShiftTo(b, l), r.subTo(l, r); r[v] < --T;) r.subTo(l, r)
            }
            null != i && (r.drShiftTo(a, i), s != h && BigInteger.ZERO.subTo(i, i)), r.t = a, r.clamp(), p > 0 && r.rShiftTo(p, r), 0 > s && BigInteger.ZERO.subTo(r, r)
        }
    }
}
...
function bnpExp(t, i) {
    ...
    var r = nbi(),
        ...
        n = i.convert(this),
        e = nbits(t) - 1;
    for (n.copyTo(r); --e >= 0;) if (i.sqrTo(r, o), (t & 1 << e) > 0) i.mulTo(o, n, r);
    else {
        var s = r;
        r = o, o = s
    }
...
function ARC4init(i) {
    ...
    for (s = 0, t = 0; 256 > t; ++t) s = s + this.S[t] + i[t % i.length] & 255, h = this.S[t], this.S[t] = this.S[s], this.S[s] = h;
    this.i = 0, this.j = 0
}
...
function rng_get_bytes(r) {
    var n;
    for (n = 0; n < r.length; ++n) r[n] = rng_get_byte()
}
...
function linebrk(n, t) {
    for (var e = "", r = 0; r + t < n.length;) e += n.substring(r, r + t) + "\n", r += t;
...
function pkcs1pad2(n, t) {
    ...
    for (var e = new Array, r = n.length - 1; r >= 0 && t > 0;) {
        ...
        128 > l ? e[--t] = l : l > 127 && 2048 > l ? (e[--t] = 63 & l | 128, e[--t] = l >> 6 | 192) : (e[--t] = 63 & l | 128, e[--t] = l >> 6 & 63 | 128, e[--t] = l >> 12 | 224)
    }
    e[--t] = 0;
...
function RSASetPublic(n, t) {
    null != n && null != t && n.length > 0 && t.length > 0 ? (this.n = parseBigInt(n, 16), this.e = parseInt(t, 16)) : alert("Invalid RSA public key")
}
...
function hex2b64(r) {
    var t, a, n = "";
    for (t = 0; t + 3 <= r.length; t += 3) a = parseInt(r.substring(t, t + 3), 16), n += b64map.charAt(a >> 6) + b64map.charAt(63 & a);
    for (t + 1 == r.length ? (a = parseInt(r.substring(t, t + 1), 16), n += b64map.charAt(a << 2)) : t + 2 == r.length && (a = parseInt(r.substring(t, t + 2), 16), n += b64map.charAt(a >> 2) + b64map.charAt((3 & a) << 4));
...
function b64tohex(r) {
    var t, a, n = "",
        h = 0;
    for (t = 0; t < r.length && r.charAt(t) != b64padchar; ++t) v = b64map.indexOf(r.charAt(t)), v < 0 || (0 == h ? (n += int2char(v >> 2), a = 3 & v, h = 1) : 1 == h ? (n += int2char(a << 2 | v >> 4), a = 15 & v, h = 2) : 2 == h ? (n += int2char(a), n += int2char(v >> 2), a = 3 & v, h = 3) : (n += int2char(a << 2 | v >> 4), n += int2char(15 & v), h = 0));
    return 1 == h && (n += int2char(a << 2)), n
}

function b64toBA(r) {
    var t, a = b64tohex(r),
        n = new Array;
    for (t = 0; 2 * t < a.length; ++t) n[t] = parseInt(a.substring(2 * t, 2 * t + 2), 16);
    return n
}
